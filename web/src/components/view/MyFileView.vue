            ...
            <el-table :data="tableData" v-loading="loading" height="680">
              ...
              >
                <template slot-scope="scope">
                  <template v-if="editIndex === scope.$index">
                    <el-input :value="scope.row.name" v-model="inputFileName">
                      <el-button
                        ...
                        slot="append"
                        type="primary"
                        @click="comformEdit(scope.$index, scope.row)"
                      >
                  ...
                  <template v-else>
                    <span @click="clickFileName(scope.$index, scope.row)">{{
                      scope.row.name
                    }}</span>
                  </template>
                </template>
...
export default {
  ...
  methods: {
      ...
      this.$http
        ...
        .then(function (response) {
          if (response.status === 200) {
            ...
            link.download = thiz.getFileNameFromHeader(
    ...
    renderHeader(createElement, { column }) {
      ...
      var createTooltip = createElement(
        "el-tooltip",
        {
          attrs: {
            content: "点击图片、PDF文件名直接跳转预览",
            placement: "top",
          },
        },
        [createItemI]
